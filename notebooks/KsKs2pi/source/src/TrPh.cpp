...
bool TrPh::cutTracks_() {
  ...
  if (trackIndices_.size() == 6) {
    int totalCharge = 0;
    for (int i = 0; i < 6; ++i) totalCharge += tcharge[trackIndices_[i]];
...
void TrPh::setupOutputBranches_(TTree *tree) {
  ...
  void TrPh::Loop(const std::string &outpath, double mfield) {
    ...
    kfcmd::hypos::Hypo2Ks2ChPions_NoKsMasses hypo(2.e-3 * emeas, mfield);
      ...
      do {
        ...
        do {
          ...
          if (!hypo.fillTrack("pi-_1", trackIndices_[mi_perm[1]], *this))
            continue;
          if (!hypo.fillTrack("pi-_2", trackIndices_[mi_perm[2]], *this))
            continue;
