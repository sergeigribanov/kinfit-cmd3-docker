...
bool TrPh::fitOnce_(kfhypos::Hypo2ChPions4Photons *hypo) {
  bool result = false;
  ...
  for (std::size_t iph = 0; iph + 3 < photonIndices_.size(); ++iph) {
    if (!hypo->fillPhoton("g0", photonIndices_[iph], *this)) continue;
    for (std::size_t jph = iph + 1; jph + 2 < photonIndices_.size(); ++jph) {
      if (!hypo->fillPhoton("g1", photonIndices_[jph], *this)) continue;
      for (std::size_t kph = jph + 1; kph + 1 < photonIndices_.size(); ++kph) {
        if (!hypo->fillPhoton("g2", photonIndices_[kph], *this)) continue;
        for (std::size_t mph = kph + 1; mph < photonIndices_.size(); ++mph) {
          if (!hypo->fillPhoton("g3", photonIndices_[mph], *this)) continue;
          hypo->optimize();
          if (hypo->getErrorCode() != 0)
            continue;
          if (hypo->getChiSquare() > tchi2)
            continue;
...
void TrPh::fillParams_(const kfhypos::Hypo2ChPions4Photons &hypo) {
...
void TrPh::Loop(const std::string &outpath, double magneticField) {
  ...
  fChain->GetEntry(0);
  Histograms hists;
  kfcmd::hypos::Hypo2ChPions4Photons hypo0(2.e-3 * emeas, magneticField, 20, 1.e-4);
  kfcmd::hypos::Hypo2ChPions2Photons hypo(2.e-3 * emeas, magneticField, 40, 1.e-4);
  ...
  hypo0.setBeamXY(xbeam, ybeam);
  hypo0.fixVertexParameter("vtx0", 0, xbeam);
  hypo0.fixVertexParameter("vtx0", 1, ybeam);
