...
void TrPh::setupOutputBranches_(TTree* tree) {
  ...
  tree->Branch("numiters", &numiters_, "numiters/I");
  ...
  tree->Branch("sigma_x_vtx1", &sigma_x_vtx1_, "sigma_x_vtx1/D");
  tree->Branch("sigma_y_vtx1", &sigma_y_vtx1_, "sigma_y_vtx1/D");
  tree->Branch("sigma_z_vtx1", &sigma_z_vtx1_, "sigma_z_vtx1/D");
  tree->Branch("q_val", &q_val_, "q_val/D");
  ...
  tree->Branch("kf_vtx_mks", &kf_vtx_mks_, "kf_vtx_mks/D");
...
void TrPh::Loop(const std::string& outpath, double magneticField) {
  ...
  fChain->GetEntry(0);
  kfcmd::hypos::HypoKsKPlusPiMinus_NoKsMass hypo_plus(2.e-3 * emeas, magneticField, 100);
  kfcmd::hypos::HypoKsKMinusPiPlus_NoKsMass hypo_minus(2.e-3 * emeas, magneticField, 100);
  kfcmd::hypos::Hypo2PiVertex hypo_vertex(2.e-3 * emeas, magneticField, 100);
    
  Eigen::MatrixXd vtx_icov = Eigen::MatrixXd::Zero(3, 3);
  vtx_icov(0, 0) = 1. / (6.e-2 * 6.e-2);
  vtx_icov(1, 1) = 1. / (6.e-2 * 6.e-2);
  hypo_plus.setVertexInverseCovarianceMatrix("vtx0", vtx_icov);
  hypo_minus.setVertexInverseCovarianceMatrix("vtx0", vtx_icov);
  ...
  double v_kf_vtx_mks_plus = 0;  
  double v_kf_mks_minus = 0;
  double v_in_mks_minus = 0;
  double v_kf_vtx_mks_minus = 0;
  ...
  double v_sigma_x_vtx0_plus;
  double v_sigma_y_vtx0_plus;
  double v_sigma_z_vtx0_plus;
  double v_sigma_x_vtx1_plus;
  double v_sigma_y_vtx1_plus;
  double v_sigma_z_vtx1_plus;

  double v_sigma_x_vtx0_minus;
  double v_sigma_y_vtx0_minus;
  double v_sigma_z_vtx0_minus;
  double v_sigma_x_vtx1_minus;
  double v_sigma_y_vtx1_minus;
  double v_sigma_z_vtx1_minus;
  ...
  for (Long64_t jentry = 0; jentry < nentries; jentry++) {
    ...
    for (int im = 0; im < 2; ++im) {
      ...
      for (int ip = 2; ip < 4; ++ip) {
        ...
        hypo_vertex.fillTrack("pi-_1", trackIndices_[im], *this);
        hypo_vertex.fillTrack("pi+_1", trackIndices_[ip], *this);
        ...
        kf_err_plus = 0;
        numiters_plus = hypo_plus.getNumOfRequiredIters();
        tchi2_plus = hypo_plus.getChiSquare();
        if (tchi2_plus < kf_chi2_plus) {
          flag_plus = true;
          kf_chi2_plus = tchi2_plus;
          Eigen::MatrixXd cov_plus = 2. * hypo_plus.getInvHessian();
          long bi_vtx0 = hypo_plus.getVertex("vtx0")->getBeginIndex();
          long bi_vtx1 = hypo_plus.getVertex("vtx1")->getBeginIndex();
          v_sigma_x_vtx0_plus = std::sqrt(std::fabs(cov_plus(bi_vtx0, bi_vtx0 )));
          v_sigma_y_vtx0_plus = std::sqrt(std::fabs(cov_plus(bi_vtx0 + 1, bi_vtx0 + 1)));
          v_sigma_z_vtx0_plus = std::sqrt(std::fabs(cov_plus(bi_vtx0 + 2, bi_vtx0 + 2)));
          v_sigma_x_vtx1_plus = std::sqrt(std::fabs(cov_plus(bi_vtx1, bi_vtx1)));
          v_sigma_y_vtx1_plus = std::sqrt(std::fabs(cov_plus(bi_vtx1 + 1, bi_vtx1 + 1)));
          v_sigma_z_vtx1_plus = std::sqrt(std::fabs(cov_plus(bi_vtx1 + 2, bi_vtx1 + 2)));


          q_val_plus = hypo_plus.getdxTHdx() - 2. * hypo_plus.getChiSquare();
          ...
          v_kf_vtx_mks_plus = hypo_vertex.getFinalMomentum(decaypds_).M();
    ...
    for (int im = 0; im < 2; ++im) {
      ...
      for (int ip = 2; ip < 4; ++ip) {
        ...
        kf_err_minus = 0;
        numiters_minus = hypo_minus.getNumOfRequiredIters();
        tchi2_minus = hypo_minus.getChiSquare();
        if (tchi2_minus < kf_chi2_minus) {
          flag_minus = true;
          kf_chi2_minus = tchi2_minus;
          Eigen::MatrixXd cov_minus = 2. * hypo_minus.getInvHessian();
          long bi_vtx0 = hypo_minus.getVertex("vtx0")->getBeginIndex();
          long bi_vtx1 = hypo_minus.getVertex("vtx1")->getBeginIndex();
          v_sigma_x_vtx0_minus = std::sqrt(std::fabs(cov_minus(bi_vtx0, bi_vtx0)));
          v_sigma_y_vtx0_minus = std::sqrt(std::fabs(cov_minus(bi_vtx0 + 1, bi_vtx0 + 1)));
          v_sigma_z_vtx0_minus = std::sqrt(std::fabs(cov_minus(bi_vtx0 + 2, bi_vtx0 + 2)));
          v_sigma_x_vtx1_minus = std::sqrt(std::fabs(cov_minus(bi_vtx1, bi_vtx1)));
          v_sigma_y_vtx1_minus = std::sqrt(std::fabs(cov_minus(bi_vtx1 + 1, bi_vtx1 + 1)));
          v_sigma_z_vtx1_minus = std::sqrt(std::fabs(cov_minus(bi_vtx1 + 2, bi_vtx1 + 2)));

          q_val_minus = hypo_minus.getdxTHdx() - 2. * hypo_minus.getChiSquare();
          ...
          v_kf_vtx_mks_minus = hypo_vertex.getFinalMomentum(decaypds_).M();
    ...
    if (flag_plus) {
      ...
      npassed[0]++;
      kf_err_ = kf_err_plus;
      numiters_ = numiters_plus;
      q_val_ = q_val_plus;
      kf_mks_ = v_kf_mks_plus;
      in_mks_ = v_in_mks_plus;
      kf_vtx_mks_ = v_kf_vtx_mks_plus;
      kf_chi2_ = kf_chi2_plus;
      in_tks_ = in_tks_plus;
      kf_tks_ = kf_tks_plus;

      sigma_x_vtx0_ = v_sigma_x_vtx0_plus;
      sigma_y_vtx0_ = v_sigma_y_vtx0_plus;
      sigma_z_vtx0_ = v_sigma_z_vtx0_plus;
      sigma_x_vtx1_ = v_sigma_x_vtx1_plus;
      sigma_y_vtx1_ = v_sigma_y_vtx1_plus;
      sigma_z_vtx1_ = v_sigma_z_vtx1_plus;
    ...
    if (flag_minus) {
      ...
      numiters_ = numiters_minus;
      kf_hypo_ = 1;
      q_val_ = q_val_minus;
      npassed[1]++;
      kf_mks_ = v_kf_mks_minus;
      in_mks_ = v_in_mks_minus;
      kf_vtx_mks_ = v_kf_vtx_mks_minus;
      kf_chi2_ = kf_chi2_minus;
      in_tks_ = in_tks_minus;
      kf_tks_ = kf_tks_minus;

      sigma_x_vtx0_ = v_sigma_x_vtx0_minus;
      sigma_y_vtx0_ = v_sigma_y_vtx0_minus;
      sigma_z_vtx0_ = v_sigma_z_vtx0_minus;
      sigma_x_vtx1_ = v_sigma_x_vtx1_minus;
      sigma_y_vtx1_ = v_sigma_y_vtx1_minus;
      sigma_z_vtx1_ = v_sigma_z_vtx1_minus;
