...
#include <iostream>
#include <set>
#include "Hypo2ChPions2PhotonsISR.hpp"
#include "TrPh.h"
...
void TrPh::Loop(const std::string& outpath, double magneticField) {
  ...
  fChain->GetEntry(0);
  kfcmd::hypos::Hypo2ChPions2PhotonsISR hypo(2.e-3 * emeas, magneticField);
  double tchi2;
  ...
  for (Long64_t jentry = 0; jentry < nentries; jentry++) {
    ...
    for (std::size_t iph = 0; iph + 1 < photonIndices_.size(); ++iph) {
      ...
      for (std::size_t jph = iph + 1; jph < photonIndices_.size(); ++jph) {
        ...
        hypo.fixParticleParameter("isr", 1, 0.);
        hypo.optimize();
        ...
        hypo.fixParticleParameter("isr", 1, M_PI);
        hypo.optimize();
        tchi2 = hypo.getChiSquare();
        if (hypo.getErrorCode() == 0) {
          if (tchi2 < kf_chi2_) {
