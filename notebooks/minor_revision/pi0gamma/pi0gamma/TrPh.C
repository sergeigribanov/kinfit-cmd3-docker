...
void TrPh::setupOutputBranches_(TTree *tree) {
  ...
  tree->Branch("sigma_vtx0_x", &sigma_vtx0_x_, "sigma_vtx0_x/D");
  tree->Branch("sigma_vtx0_y", &sigma_vtx0_y_, "sigma_vtx0_y/D");
...
void TrPh::fit_(Hypo3PhotonsCustom *hypo) {
  ...
  for (std::size_t iph = 0; iph + 2 < photonIndices_.size(); ++iph)
    ...
    sigma_vtx0_x_ = std::sqrt(std::fabs(cov(bi_vtx0, bi_vtx0)));
    sigma_vtx0_y_ = std::sqrt(std::fabs(cov(bi_vtx0 + 1, bi_vtx0 + 1)));
...
void TrPh::Loop(const std::string& outpath, double mfield) {
  ...
  hypo.setVertexInverseCovarianceMatrix("vtx0", vtx_icov);
