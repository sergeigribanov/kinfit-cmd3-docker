...
void TrPh::refit_(kfhypos::Hypo2ChPions2Photons *hypo, Histograms* hists) {
  ...
  hypo->setInitialVertexParams("vtx0", gaussgen::gaussgen_nfirst(2, params_.parVtx0, params_.invCovVtx0, &rnd_));
  hypo->optimize();
...
void TrPh::Loop(const std::string &outpath, double magneticField) {
  ...
  fChain->GetEntry(0);
  Histograms hists;
  kfcmd::hypos::Hypo2ChPions2Photons hypo(2.e-3 * emeas, magneticField, 200, 1.e-9);
