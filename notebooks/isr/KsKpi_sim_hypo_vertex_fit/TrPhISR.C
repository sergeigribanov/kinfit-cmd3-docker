...
void TrPh::Loop(const std::string& outpath, double magneticField) {
  ...
  fChain->GetEntry(0);
  kfcmd::hypos::HypoKsKPlusPiMinus_NoKsMass_ISR hypo_plus(2.e-3 * emeas, magneticField);
  kfcmd::hypos::HypoKsKMinusPiPlus_NoKsMass_ISR hypo_minus(2.e-3 * emeas, magneticField);
  ...
  for (Long64_t jentry = 0; jentry < nentries; jentry++) {
    ...
    for (int im = 0; im < 2; ++im) {
      ...
      for (int ip = 2; ip < 4; ++ip) {
        ...
        hypo_plus.fixParticleParameter("isr", 1, 0);
        hypo_plus.optimize();
        ...
        tmpv << ks_im.Px(), ks_im.Py(), ks_im.Pz(), 1.e-3;
        hypo_plus.setInitialParticleParams("ks", tmpv);
        hypo_plus.fixParticleParameter("isr", 1, M_PI);
        hypo_plus.optimize();
        errCode = hypo_plus.getErrorCode();
        if (errCode == 0) {
          kf_err_plus = 0;
    ...
    for (int im = 0; im < 2; ++im) {
      ...
      for (int ip = 2; ip < 4; ++ip) {
        ...
        hypo_minus.fixParticleParameter("isr", 1, 0);
        hypo_minus.optimize();
        ...
        tmpv << ks_im.Px(), ks_im.Py(), ks_im.Pz(), 1.e-3;
        hypo_minus.setInitialParticleParams("ks", tmpv);
        hypo_minus.fixParticleParameter("isr", 1, M_PI);
        hypo_minus.optimize();
        errCode = hypo_minus.getErrorCode();
        if (errCode == 0) {
          kf_err_minus = 0;
