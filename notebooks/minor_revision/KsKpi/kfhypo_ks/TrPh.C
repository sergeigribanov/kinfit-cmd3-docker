...
double TrPh::getAngle_(const kfhypos::HypoKsKPlusPiMinus &hypo) const {
  auto dv = hypo.getFinalVertex("vtx1") - hypo.getFinalVertex("vtx0");
  auto p = hypo.getFinalMomentum(decaypds_).Vect();
  return dv.Angle(p);
}

double TrPh::getAngle_(const kfhypos::HypoKsKMinusPiPlus &hypo) const {
  auto dv = hypo.getFinalVertex("vtx1") - hypo.getFinalVertex("vtx0");
  auto p = hypo.getFinalMomentum(decaypds_).Vect();
  return dv.Angle(p);
}
...
void TrPh::Loop(const std::string& outpath, double magneticField) {
  ...
  fChain->GetEntry(0);
  kfcmd::hypos::HypoKsKPlusPiMinus hypo_plus(2.e-3 * emeas, magneticField, 100);
  kfcmd::hypos::HypoKsKMinusPiPlus hypo_minus(2.e-3 * emeas, magneticField, 100);
