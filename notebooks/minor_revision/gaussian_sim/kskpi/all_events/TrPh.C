...
Histograms::Histograms() {
  chi2Hist = new TH1F("kf_chi2", "", 256, 0, 150);
  qHist = new TH1F("qhist", "", 1024, -512, 512);
  chi2_vs_q = new TH2F("chi2_vs_q", "", 1024, 0, 1024, 1024, -512, 512);
}
...
bool TrPh::refitKPlus_(kfcmd::hypos::HypoKsKPlusPiMinus_NoKsMass *hypo,
                       Histograms *hists) {
  ...
  hypo->setInitialParticleParams("ks", tmpv);
  hypo->optimize();
  if (hypo->getErrorCode() != 0) return false;
...
bool TrPh::refitKMinus_(kfcmd::hypos::HypoKsKMinusPiPlus_NoKsMass *hypo,
                       Histograms *hists) {
  hypo->setInitialParticleParams("pi-_1",
                                 gaussgen::gaussgen_nfirst(5, params_.parPiMinus1,
                                                           params_.invCovPiMinus1,
...
void TrPh::Loop(const std::string &outpath, double magneticField) {
  ...
  for (int entry = 0; entry < nentries; ++entry) {
    ...
    switch (sim_hypo_) {
    case 0:
      if (!fitOnceKPlus_(&hypo_plus)) continue;
      k = 0;
      do {k++;} while (refitKPlus_(&hypo_plus, &hists) != true && k < 1000);
      break;
    case 1:
      if (!fitOnceKMinus_(&hypo_minus)) continue;
      k = 0;
      do {k++;} while (refitKMinus_(&hypo_minus, &hists) != true && k < 1000);
      break;
    default:
      break;
    }
